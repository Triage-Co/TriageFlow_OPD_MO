...
export default function PackageBookingScreen() {
  ...
  // Tạo danh sách 7 ngày từ ngày hôm nay
  ...
  // Chọn ngày mặc định khi mở màn hình
  useEffect(() => {
  ...
  // Gọi API tải slots khi đổi ngày
  ...
  const handleRegisterPackage = async () => {
    ...
    try {
      ...
      // Nhận booking code/ID thành công
            ...
            <View className="bg-white rounded-[32px] p-6 items-center text-center shadow-2xl max-w-sm w-full">
              ...
              <Pressable
                onPress={() => {
                  ...
                  router.dismissAll(); // Close the stack
                  router.replace("/(patient)/(tabs)/home"); // Back to home
                }}
